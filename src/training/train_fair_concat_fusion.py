...
class FairFusionDataset(Dataset):
    def __init__(self, csv_path: Path, labels: list[str], expected_structured_dim: int = 531) -> None:
        self.df = pd.read_csv(csv_path)
        self.labels = labels
        self.feature_cols = [col for col in self.df.columns if col.startswith("signal_emb_") or col.startswith("struct_")]
        missing = [col for col in ["ecg_id", "split"] + labels if col not in self.df.columns]
        if missing:
            raise ValueError(f"{csv_path} is missing required columns: {missing}")
        if not self.feature_cols:
            raise ValueError(f"{csv_path} has no fusion feature columns.")
        struct_dim = len([col for col in self.feature_cols if col.startswith("struct_")])
        if struct_dim != expected_structured_dim:
            raise ValueError(f"Fair concat expected {expected_structured_dim} structured features, got {struct_dim}")
...
@torch.no_grad()
def _predict(model: nn.Module, dataset: FairFusionDataset, loader: DataLoader, device: torch.device, labels: list[str]) -> tuple[np.ndarray, np.ndarray, pd.DataFrame]:
    model.eval()
...
def train(config_path: str | Path, seed_override: int | None = None, output_dir_override: str | Path | None = None) -> int:
    ...
    if output_dir_override is not None:
        ...
        config = copy.deepcopy(config)
        config["output"] = {
            "val_predictions_csv": str(out_dir / "fair_concat_val_predictions.csv"),
            "test_predictions_csv": str(out_dir / "fair_concat_test_predictions.csv"),
            "history_csv": str(out_dir / "fair_concat_training_history.csv"),
            "metrics_val_csv": str(out_dir / "fair_concat_metrics_val.csv"),
            "metrics_test_csv": str(out_dir / "fair_concat_metrics_test.csv"),
            "thresholds_json": str(out_dir / "fair_concat_thresholds.json"),
            "run_summary_json": str(out_dir / "fair_concat_run_summary.json"),
            "checkpoint_path": str(out_dir / "fair_concat_best.pt"),
            "table_csv": str(out_dir / "table_fair_concat_results.csv"),
            "threshold_comparison_csv": str(out_dir / "table_fair_concat_threshold_comparison.csv"),
        }
    ...
    model_name = str(model_cfg.get("name", "fair_concat_mlp"))
    ...
    train_ds = FairFusionDataset(paths["train_csv"], labels, expected_structured_dim=expected_structured_dim)
    val_ds = FairFusionDataset(paths["val_csv"], labels, expected_structured_dim=expected_structured_dim)
    test_ds = FairFusionDataset(paths["test_csv"], labels, expected_structured_dim=expected_structured_dim)
    input_dim = len(train_ds.feature_cols)
    signal_dim = len([col for col in train_ds.feature_cols if col.startswith("signal_emb_")])
    struct_dim = len([col for col in train_ds.feature_cols if col.startswith("struct_")])
    if signal_dim <= 5:
        raise ValueError(f"Fair concat requires high-dimensional signal embedding, got {signal_dim}")
    ...
    model = FusionMLP(input_dim=input_dim, hidden_dims=model_cfg.get("hidden_dims", [512, 256]), num_classes=len(labels), dropout=float(model_cfg.get("dropout", 0.3))).to(device)
    ...
    torch.save({"model_state_dict": model.state_dict(), "config": config, "labels": labels, "input_dim": input_dim, "best_epoch": best_epoch, "best_validation_score": best_score}, paths["checkpoint_path"])
    summary = {
        ...
        "n_val": len(val_ds),
        "n_test": len(test_ds),
        "signal_embedding_dim": signal_dim,
        "structured_feature_dim": struct_dim,
    ...
    print(f"Fair concat MLP completed. best_epoch={best_epoch} val_macro_auroc={best_score:.6f}")
    print(f"Table: {paths['table_csv']}")
    return 0


def main() -> None:
    parser = argparse.ArgumentParser(description="Train fair MLP concat fusion baseline.")
    parser.add_argument("--config", default="configs/model_fair_concat_mlp.yaml")
